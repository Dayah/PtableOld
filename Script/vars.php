...
?>
	quickstring = [];
	for (var x in electronstring) if (electronstring[x]) quickstring[x] = electronstring[x].replace(/(\d+)(\d[spdf]|$)/g, "<sup>$1<\/sup> $2");
...
function get_cookies() {
	var cookiearray = document.cookie.split("; "), resultcookie = [];
	for (var x in cookiearray) {
...
function cache_objects() {
	...
	var temp_series = document.getElementById("Series").getElementsByTagName("td");
	for (var x = 0; x < temp_series.length; x++)
		if (temp_series[x].id)
			series_ids[temp_series[x].id] = temp_series[x];
	var temp_block = document.getElementById("Block").getElementsByTagName("th");
	for (var x = 0; x < temp_block.length; x++)
...
function resize_hunt() {
	var sheet_ids = ["Weight", "Name", "Electrons"], index = -1, before = [], after = [];
	for (var x in sheet_ids) before[x] = document.getElementById(sheet_ids[x]).checked;
...
function attach_class_hovers() {
	...
	collect_class = "";
	for (var x = 5; x < 8; x++) collect_class += row[1].cells[x].className + " ";
	...
	if (typeof document.getElementById("Series").onmouseleave !== "undefined")
		...
		document.getElementById("Series").onmouseout = document.getElementById("Block").onmouseout = document.getElementById("DecayModes").onmouseout = function (e) {
			for (var el = e.relatedTarget; el && (el != document.getElementById("Series") && el != document.getElementById("Block") && el != document.getElementById("DecayModes")); el = el.parentNode);
			if (!el) series_leave();
		};
	}
	...
	if (typeof matterstate.onmouseleave !== "undefined")
		...
		matterstate.onmouseout = function (e) {
			for (var el = e.relatedTarget; el && (el != matterstate); el = el.parentNode);
			if (!el) unhover_state();
		};
...
function dataset_changed(update, el) {
	var el = (el || this || srcElement).getElementsByTagName("input");
...
function color_other(valueArray, specArray) {
	var minmax = getminmax(valueArray, specArray["scale"][specArray["subset"]]);
	for (var i = 1; i <= 118; i++) {
...
function init_layout() {
	element_ids = [];
	var tags = document.getElementsByTagName("td"), atomic;
...
function click_checkbox(obj) {
	...
	if (obj.id === "wide") {
		...
		var sheet_ids = {"Electrons": 0, "Name": 1, "Weight": 2}, reverse_ids = ["Electrons", "Name", "Weight"];
		for (var i = sheet_ids[obj.id]; Math.abs(i - 1) < 2; i += (obj.checked - 0.5) * 2)
			document.getElementById(reverse_ids[i]).checked = obj.checked;

		var sheets_list = document.getElementsByTagName("link");
		for (var x = 2; x < 5; x++) {
	...
	var checkbox_ids = ["Weight", "Name", "Electrons", "wide"];
	for (var x in checkbox_ids)
...
function cookie_checkbox(save, id) {
	...
	if (save) {
		...
		else for (var x in checkbox_ids)
...
function state_classes(updateall, flash_updated) {
	var value = temperature, tempcolor;
	for (var i = 1; i <= 118; i++) {
		tempcolor = updateall;
...
function colorize_and_mass(includemass) {
	var attr = window[dataset], spec = window["spec_" + dataset];
	if (includemass) {
		if (dataset === "state") {
...
function insert_oxidation(convert) {
	if (convert) {
		for (var i = 1; i <= 118; i++) if (orbital[1][i]) {
		...
		if (topes.length) {
			...
			switch (e.keyCode) {
				...
				case 38: case 40:
					...
					var row = wholetable.tBodies[0].rows[rowind - (20 - e.keyCode / 2 + (element_ids[active].cellIndex < 3 ? 1 : 0))];
...
function group_period(el, color) {
	...
	if (el.cellIndex < 3) var count = el.cellIndex;
	else {
		var count = 20 - wholetable.tBodies[0].rows[period].cells.length + el.cellIndex + widecheck.checked + (wholetable.tBodies[0].rows[period].cells[3].cellIndex !== 3);
...
function hover_group(e) {
	...
	for (var period = min; period < max; period++) {
		if (count <= 3) var index = count;
		else {
			var index = wholetable.tBodies[0].rows[period].cells.length - 20 + count - widecheck.checked;
...
function hover_period(e) {
	...
	var end = (tab === "OrbitalTab" && start === 1 ? 2 : wholetable.rows[period].cells[wholetable.rows[period].cells.length - 2].idx);
...
function click_isotope(oXML, atomic, fast) {
	...
	var returned_data = isotopecache[atomic].responseText.split("\n");
...
function draw_isotope(atomic, count, specs, fast) {
	for (var i in datasets["IsotopeTab"])
