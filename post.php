  ...
  <table id="Main" summary="printable periodic table" cellspacing=0>
   <tr>
    ...
    <td id="HideEC" colspan=15 rowspan=2>
     <form method="get" action="./">
<? if (array_key_exists("lang", $_GET)) { ?><input type="hidden" name="lang" value="<?= $_GET["lang"] ?>"><? }

if ($layout == "electron") { ?>
<input type="submit" value="<?= local("Hide N") ?>">
<? } elseif ($layout == "names") { ?>
<input type="hidden" name="layout" value="electron">
<input type="submit" value="<?= local("Show EC") ?>">
<? } else { ?>
<input type="hidden" name="layout" value="names">
<input type="submit" value="<?= local("Show N") ?>">
<? }
   if (array_key_exists("scheme", $_GET)) { ?><input type="hidden" name="scheme" value="<?= $scheme ?>"><? } ?>
     </form>
    </td>
